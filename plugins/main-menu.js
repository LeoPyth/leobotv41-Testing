 ...
 if (teks == 'edukasi') tags = {
  ...
  try {
      ...
      const sections = [
   {
	title: `Status`,
	rows: [
	    {title: `🎟 ¦ INFO BOT`, rowId: ".info", description: `Info nya ${namebot}`},
	...
	{title: `🗃 ¦Bot Stats`, rowId: ".botinfo", description: `Menampilkan Status ${namebot}`},
	{title: `🕯 ¦ Test Speed`, rowId: ".testspeed", description: "Test Install Speed BOT"},
	{title: `🚀 ¦ Speed`, rowId: ".speed", description: `Kecepatan Respon ${namebot}`},
	]
    },{
	title: `Category`,
	rows: [
	    {title: `🏷 ¦ Sewa BOT`, rowId: ".sewabot", description: "Menampilkan list harga sewa BOT"},
	    {title: `🗿🚬 Sc Ini Direcode Oleh Leo `, rowId: "p", description: "Mau Esceh kan?"},
	...
	title: `List Menu`,
	rows: [
	    {title: `💬 | SEMUA PERINTAH`, rowId: ".? all", description: "Semua fitur ada disini"},
	...
	{title: `⛩️ ¦ Anime`, rowId: ".? anime", description: "Lah, Wibuu"},
	...
	{title: `⚙️ ¦ Group`, rowId: ".? group", description: "Menu settingan buat group mu kak"},
	{title: `⚠️ ¦ Admin`, rowId: ".? admin", description: "Yg bukan admin jangan make fitur ini"},
	...
	{title: `⚠️ ¤ fitur owner`, rowId: ".? owner", description: "Yg bukan developer gausah nge klik fitur ini !"},
	{title: `📖 ¦ Maker`, rowId: ".? maker", description: "Bikin logo apa ya enaknya?"},
	{title: `🛒 ¦ Store`, rowId: ".? store", description: "Anak jebeh ya kamu kak?"},
	{title: `📮 ¦ Confess`, rowId: ".menfess", description: "Wow ada yang mau confes nih"},
	]
    },{
	title: `info`,
	rows: [
	    ...
	    {title: `📔 ¦ Script`, rowId: ".sc2", description: `Source Code ${namebot}`},
...
let tek = `
 ...
 │⬡ Premium : *${usrs.premiumTime > 1 ? 'Iya': 'Timdak'}* 
 │⬡ Nama : *${await conn.getName(m.sender)}*
 │⬡ Status : *${m.sender.split`@`[0] == nomorown ? 'Developer' : (usrs.premiumTime >= 1 ? 'Pengguna Premium' : 'Pengguna Gratisan Awokawok')}* 
...
  } catch (e) { 
 ...
 handler.command = /^(menu|p|list|help|\?)$/i 
...
function ucapan() {
  ...
  let res = "Selamat DiniHari, Tidur Sana! ☀️"
  if (time >= 4) {
    res = "Selamat Pagi Banh 🌄"
  }
  if (time >= 10) {
    res = "Selamat Siang Banh ☀️"
  }
  if (time >= 15) {
    res = "Selamat Sore Banh 🌇"
  }
  if (time >= 18) {
    res = "Selamat Malam Banh 🌙"
  }
  return res
}